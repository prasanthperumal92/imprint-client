				...
				data.save(function(err) {
					if (err) {
						...
						Log.addLog({
							...
							clientId: user._id,
							text: 'Created a ' + configs.details['Bterm'],
							type: configs.details['Bterm'],
							by: user.employee.name,
							created: new Date()
						});
						let text = user.employee.name + ' has assigned a ' + configs.details['Bterm'] + ' to You';
					...
					function(err, newData) {
						if (err) {
							...
							Log.addLog({
								userId: user.employee._id,
								clientId: user._id,
								text: 'Updated the ' + configs.details['Bterm'],
								type: configs.details['Bterm'],
								by: user.employee.name,
								created: new Date()
							});
