...
exports.clientList = function(req, res, next) {
	...
	if (!id) {
		...
		if (user.employee.type == 'manager') {
			let tmp = [],
				empId = user.employee._id;
			tmp.push(empId);
			User.findById({ _id: user._id }, function(err, emps) {
				if (err) {
					return res.status(401).send({
						message: 'Error looking up User Information'
					});
				} else {
					console.log('sdasd asdasd', emps);
					for (let i = 0; i < emps.employees.length; i++) {
						if (emps.employees[i].reportingTo.equals(empId)) {
							tmp.push(emps.employees[i]._id);
						}
					}
				}
				query = {
